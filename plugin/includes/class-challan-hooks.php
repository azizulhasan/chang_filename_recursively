...
class Challan_Hooks
{
    ...
    public function __construct() {
        if ( ! empty(get_option('wpifw_invoicing')) ) {
            ...
            add_filter('handle_bulk_actions-edit-post', array( $this, 'challan_bulk_action_handler' ), 10, 3);
            // ###################################### END ##########################################
          ...
          //  add_action('woocommerce_admin_order_actions_end', array( $this, 'add_packing_slip_order_action_button' ));
	        ...
	        add_action( 'wp_ajax_wpifw_generate_delivery_address', array( $this, 'challan_pro_generate_delivery_address' ) );
            ...
            add_action('wp_ajax_challan_font_download_ajax', array( $this, 'challan_font_download_ajax' ));
        }
    }
	...
	public function challan_font_download_ajax() {
		...
		if ( ! empty( $get_font_content ) ) {
		    ...
		    if ( '.zip' !== $get_last_extension ) {
			    ...
			    if ( in_array($file_name, $array_collection) ) {
				    file_put_contents( CHALLAN_FONT_DIR . $file_name, $get_font_content );
		       ...
		       //  Upload zip file.
			    ...
			    if ( class_exists('ZipArchive') ) {
				    $zip = new ZipArchive();
				    if ( $zip->open( CHALLAN_FONT_DIR . $file_name ) === true ) {
					    $zip->extractTo( CHALLAN_FONT_DIR );
					    $zip->close();
					    unlink( CHALLAN_FONT_DIR . $file_name );
					    $response = array(
						    'font_name' => $file_name,
					    );
					    wp_send_json_success( $response );
    ...
    public function add_my_account_order_action_download_invoice( $actions, $order ) {
        ...
        if ( in_array( 'always_allow', $wpifw_invoice_download_check_list )
            ...
            $actions['wpifw-my-account-invoice'] = array(
                'url'   => wp_nonce_url( admin_url( 'admin-ajax.php?action=wpifw_generate_invoice&order_id=' . $order_id.$output_type ), 'challan_ajax_nonce' ),
    ...
    public function add_custom_meta_box() {
        add_meta_box('wpifw-meta-box', 'Challan', array( $this, 'pdf_meta_box_markup' ), 'shop_order', 'side', 'high', null);
    }
    ...
    public function pdf_meta_box_markup() {
        global $post;
        $invoice      = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_invoice&order_id=' . $post->ID), 'challan_ajax_nonce');
        $packing_slip = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_invoice_packing_slip&order_id=' . $post->ID), 'challan_ajax_nonce');
	    $delivery_address = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_delivery_address&order_id=' . $post->ID), 'challan_ajax_nonce');
        ?>
     ...
     * @return string
     */
    public function challan_bulk_action_handler( $redirect_to, $doaction, $post_ids ) {
    ...
    public function add_invoice_order_action_button( $order ) {
        ...
        // Packing slip button.
	    $url         = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_invoice_packing_slip&order_id=' . $order_id.$output_type), 'challan_ajax_nonce');
	    ...
	    $url         = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_delivery_address&order_id=' . $order_id.$output_type), 'challan_ajax_nonce');
...
//        $order_id = method_exists($order, 'get_id') ? $order->get_id() : '';
//
//        $url         = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_invoice_packing_slip&order_id=' . $order_id), 'challan_ajax_nonce');
//        $text        = __('Packing Slip', 'webappick-pdf-invoice-for-woocommerce');
//        $class       = 'tips parcial wpifw_invoice_packing_slip wpifw_button_invoice_packing_slip';
//        $src         = CHALLAN_FREE_PLUGIN_URL . 'admin/images/shipping_list.svg';
//        $image_class = 'width: 19px;margin-top: 4px;margin-left: 3px;';
...
//   }
    ...
    public function attach_invoice_to_order_email( $attachments, $status, $order ) {
        ...
        $allowed_statuses = apply_filters( 'challan_email_types', $allowed_statuses );

        // Delete old pdf files before generating new one.
        array_map( 'unlink', glob( CHALLAN_DIR . '*.pdf' ) ); // Delete files.

        // Generate & Save Invoice.
        challan_engine()->save_invoice($order_id);

        // Attach invoice with email.
        $invoice_no = challan_get_invoice_number( $order_id );
        $file_name  = 'Invoice-' . $invoice_no;
        $file_name  = apply_filters( 'challan_file_name', $file_name, 'invoice', $order_id );

        // Return file path.
        if ( isset( $status ) && in_array( $status, $allowed_statuses ) ) {
            $pdf_path      = CHALLAN_DIR . $file_name . '.pdf';
    ...
    public function add_invoice_download_link( $order ) {
        ...
        if ( ( isset($status) && in_array($status, $allowed_statuses, true) ) ) {
            $invoice_download_link = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_invoice&order_id=' . $order_id), 'challan_ajax_nonce');
            $download_invoice_text = get_option('wpifw_DOWNLOAD_INVOICE_TEXT');
            $download_invoice_text = ( $download_invoice_text ) ? $download_invoice_text : 'Download Invoice';
     ...
     * @return void
     */
    public function challan_download_view_order_page( $order ) {
        $order_id = $order->get_id();
        $url      = wp_nonce_url(admin_url('admin-ajax.php?action=wpifw_generate_invoice&order_id=' . $order_id), 'challan_ajax_nonce');
    ...
    public function generate_invoice() {
        ...
        if ( ! wp_verify_nonce($retrieved_nonce, 'challan_ajax_nonce') ) {
        ...
        challan_engine()->generate_invoice($order_ids);
    }
    ...
    public function generate_packing_slip() {
        ...
        challan_engine()->generate_packing_slip($order_ids);
    }
	 ...
	 */
	public function challan_pro_generate_delivery_address() {
	    // Check use is login.
		if ( ! is_user_logged_in() ) {
			auth_redirect();
			exit;
		...
		challan_engine()->generate_delivery_address($order_ids);
    ...
    public function save_pdf_template() {
        check_ajax_referer('challan_ajax_nonce');
        $template = array(
