 ...
 * @since 1.0.0
 *
 * @package    Challan_Helper
 * @subpackage Challan_Helper/includes
 */
...
class Challan_Template {
	...
	/**
	 * Challan_Template constructor.
	 *
	 * @param array $order_ids Order Id or Ids.
	 * @param string $template_type Template Type.
	 */
	public function __construct( $order_ids, $template_type = 'invoice' ) {
		$this->helper        = challan_helper();
		$this->orders        = challan_orders( $order_ids, $template_type );
	...
	public function get_css() {
        ...
        <head>
            ...
            <style>
                <?php
				$custom_css = challan_custom_style($this->template_type); // Load custom css from action hook.
...
<div dir="<?php echo $this->rtl; ?>" class='invoice-box <?php echo $page_break; //phpcs:ignore?>'>
	<?php
	echo challan_before_document( $this->order, $this->template_type );//phpcs:ignore
	$html = ob_get_contents();
	ob_end_clean();
	...
	public function get_header_section() {
		...
		$seller = apply_filters( 'challan_seller_info', $seller, $this->template_type );
		// Action Before the seller address.
		$seller_before = challan_before_seller_info( $this->order, $this->template_type );
		// Action After the seller address.
		$seller_after = challan_after_seller_info( $this->order, $this->template_type );
		?>
	...
	public function get_order_section( $order ) {
		$billing  = apply_filters( 'challan_billing_info', $order['billing_info'], $this->order, $this->template_type );
		$shipping = apply_filters( 'challan_shipping_info', $order['shipping_info'], $this->order, $this->template_type );

		// Action Before the billing address.
		$billing_before = challan_before_billing_address( $this->order, $this->template_type );
		// Action After the billing address.
		$billing_after = challan_after_billing_address( $this->order, $this->template_type );
		// Action Before the shipping address.
		$shipping_before = challan_before_shipping_address( $this->order, $this->template_type );
		// Action After the shipping address.
		$shipping_after = challan_after_shipping_address( $this->order, $this->template_type );
		// Action Before the order data.
		$before_order_data = challan_before_order_data( $this->order, $this->template_type );
		// Action After the order data.
		$after_order_data = challan_after_order_data( $this->order, $this->template_type );
		$order_data       = $order['order_info'];
		ob_start();

		$billing  = ! empty( $billing ) ? '<b>' . challan_filter_label('Billing',$this->order,$this->template_type) . '</b><br/>' . $billing : ''; //phpcs:ignore
		$shipping = ! empty( $shipping ) ? '<b>' . challan_filter_label('Shipping',$this->order,$this->template_type) . '</b><br/>' . $shipping : ''; //phpcs:ignore
		?>
            ...
            <tr>
                ...
                <td class="order-data-td">
                    <table class="order-data-table">
						...
						?>
                        <tr>
                            <td class="order-data-label"><?php echo esc_html( challan_filter_label('Invoice Number', $this->order, $this->template_type) ); ?></td>
                            <td class="order-data-value"><?php echo ': ' . $order_data['invoice_number']; //phpcs:ignore?></td>
                        </tr>
                        <tr>
                            <td class="order-data-label"><?php echo esc_html( challan_filter_label("Order Number", $this->order, $this->template_type) );?></td>
                            <td class="order-data-value"><?php echo ': ' . $order_data['order_number']; //phpcs:ignore?></td>
                        </tr>
                        <tr>
                            <td class="order-data-label"><?php echo esc_html( challan_filter_label("Order Date", $this->order, $this->template_type) );?></td>
						...
						if ( ! empty( $order_data['payment_method'] ) ) {
							?>
                            <tr>
                                <td class="order-data-label"><?php echo esc_html( challan_filter_label("Payment Method",$this->order,$this->template_type) ); ?></td>
	...
	public function get_product_section( $products ) {
		...
		$before_product_list = challan_before_product_list( $this->order, $this->template_type );
		echo esc_html( $before_product_list );
		?>
        <table border="" class="product-table">
            <thead>
            <tr class="product-list-header">
                <th class="product-column"><?php echo esc_html( challan_filter_label("Item", $this->order, $this->template_type) ); ?></th>
				<?php if ( 'invoice' === $this->template_type ) : ?>
                    <th class="price-column"><?php echo esc_html( challan_filter_label('Cost', $this->order, $this->template_type) ); ?></th>
                    <th class="quantity-column"><?php echo esc_html( challan_filter_label('Qty', $this->order, $this->template_type) ); ?></th>
                    <th class="total-column"><?php echo esc_html( challan_filter_label('Total', $this->order, $this->template_type) ); ?></th>
				<?php elseif ( 'packing_slip' === $this->template_type ) : ?>
                    <th class="weight-column"><?php echo esc_html( challan_filter_label('Weight', $this->order, $this->template_type) ); ?></th>
                    <th class="quantity-column"><?php echo esc_html( challan_filter_label('Quantity', $this->order, $this->template_type) ); ?></th>
				<?php endif; ?>
            </tr>
            </thead>
		...
		$after_product_list = challan_after_product_list( $this->order, $this->template_type );
	...
	public function get_product_total_section( $order ) {
		...
		$product_total .= "<tr><td rowspan='$row_span' class='paid-stamp'></td> <td class='order-total-label subtotal-label'>" . esc_html( challan_filter_label('Items Subtotal', $this->order, $this->template_type) ) . " :</td><td class='order-total-value subtotal-value'>" . $total['subtotal'] . '</td></tr>';

		if ( ! empty( $total['fees'] ) ) {
			$product_total .= "<tr><td class='order-total-label fees-label'>" . esc_html( challan_filter_label('Fees', $this->order, $this->template_type) ) . " :</td><td class='order-total-value fees-value'>" . $total['fees'] . '</td></tr>'; //phpcs:ignore
		}

		if ( ! empty( $total['shipping_total'] ) ) {
			$product_total .= "<tr><td class='order-total-label shipping-label'>" . esc_html( challan_filter_label('Shipping', $this->order, $this->template_type) ) . " :</td><td class='order-total-value shipping-value'>" . $total['shipping_total'] . '</td></tr>'; //phpcs:ignore
		}

		if ( ! empty( $total['tax_total'] ) ) {
			$product_total .= "<tr><td class='order-total-label tax-label'>" . esc_html( challan_filter_label('Tax', $this->order, $this->template_type) ) . " :</td><td class='order-total-value tax-value'>" . $total['tax_total'] . '</td></tr>';
		}
		...
		$product_total .= "<tr><td class='order-total-label'>" . esc_html( challan_filter_label('Order Total', $this->order, $this->template_type) ) . " :</td><td class='order-total-value'>" . $order_total . '</td></tr>'; //phpcs:ignore
		if ( ! empty( $refund ) ) {
			$product_total .= "<tr><td class='order-total-label '>" . esc_html( challan_filter_label('Refund', $this->order, $this->template_type) ) . " :</td><td class='order-total-value  refund'>-" . $refund . '</td></tr>'; //phpcs:ignore
			$product_total .= "<tr><td class='order-total-label '>" . esc_html( challan_filter_label('Net Payment', $this->order, $this->template_type) ) . " :</td><td class='order-total-value'>" . $net_total . '</td></tr>'; //phpcs:ignore
		}
	...
	public function get_order_note_section( $order_note ) {
		...
		$before_order_note = challan_before_customer_notes( $this->order, $this->template_type );
		// Action after order note.
		$after_order_note = challan_after_customer_notes( $this->order, $this->template_type );

		if ( ! empty( $order_note ) ) {
			$order_note = '<b>' . esc_html( challan_filter_label('Customer Note', $this->order, $this->template_type) ) . ':</b> ' . $order_note;
		}

		if ( 'invoice' === $this->template_type && $wpifw_show_order_note ) {
			if ( has_filter( 'challan_customer_notes' ) ) {
				$order_note = apply_filters( 'challan_customer_notes', $order_note, $this->template_type, $this->order );
			}
	...
	public function get_packing_total_section( $order ) {
		...
		if ( ! empty( $packing_total ) ) {
			...
			$product_total          .= "<tr><td class='order-total-label'>" . esc_html( challan_filter_label('Total Quantity', $this->order, $this->template_type) ) . " :&nbsp;&nbsp;</td><td class='order-total-value'>" . $product_total_quantity . '</td></tr>';
			if ( $weight_weight > 0 ) {
				$product_total .= "<tr><td class='order-total-label'>" . esc_html( challan_filter_label('Total Weight', $this->order, $this->template_type) ) . " :&nbsp;&nbsp;</td><td class='order-total-value'>" . $weight_weight . '</td></tr>';
			}
...
if ( 'invoice' === $this->template_type ) {
	...
	if ( has_filter( 'challan_footer_1' ) ) {
		$terms_and_condition = apply_filters( 'challan_footer_1', $terms_and_condition, $this->template_type, $this->order );
	}
	// Footer 2.
	$other_information = stripslashes( get_option( 'wpifw_other_information' ) );
	if ( has_filter( 'challan_footer_2' ) ) {
		$other_information = apply_filters( 'challan_footer_2', $other_information, $this->template_type, $this->order );
	}
	...
	?>
        ...
        <div class="invoice-footer">
			...
			$footer_content = challan_after_document( $this->order, $this->template_type );
 ...
 *
 * @return Challan_Template
 */
function challan_template( $order_ids, $template ) {
	return new Challan_Template( $order_ids, $template );
}
