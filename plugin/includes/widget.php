...
if ( ! function_exists( 'challan_add_dashboard_widgets' ) ) {
	/**
	 * Add a widget to the dashboard.
	 *
	 * This function is hooked into the 'wp_dashboard_setup' action below.
	 */
	function challan_add_dashboard_widgets() {
		global $wp_meta_boxes;

		add_meta_box( 'aaaa_webappick_latest_news_dashboard_widget', __( 'Latest News from WebAppick Blog', 'webappick-pdf-invoice-for-woocommerce' ), 'challan_dashboard_widget_render', 'dashboard', 'side', 'high' );

	}
	add_action( 'wp_dashboard_setup', 'challan_add_dashboard_widgets', 1 );
}

if ( ! function_exists( 'challan_dashboard_widget_render' ) ) {
	/**
	 * Function to get dashboard widget data.
	 */
	function challan_dashboard_widget_render() {
