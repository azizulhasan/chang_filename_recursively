...
class Chalan_Notifications{
	...
	public function notifications_load_hooks(){
		add_action( 'admin_notices', [ $this, 'challan_review_notice' ] );
		add_action('wp_ajax_challan_save_review_notice', [ $this, 'challan_save_review_notice' ] );
		add_action('wp_ajax_challan_hide_notice', [ $this, 'challan_hide_notice' ] );
    }
	/**
	 * Review notice action.
	 */
	public function challan_review_notice() {

//	    delete_option('challan_review_notice_next_show_time');
//	    delete_user_meta('1', 'challan_review_notice_dismissed');

		$nonce         = wp_create_nonce( 'challan_notice_nonce' );
		$pluginName    = sprintf( '<b>%s</b>', esc_html__( 'Challan', 'webappick-pdf-invoice-for-woocommerce' ) );
		$has_notice    = false;
		$user_id       = get_current_user_id();
		$next_timestamp = get_option('challan_review_notice_next_show_time');
		$review_notice_dismissed = get_user_meta( $user_id, 'challan_review_notice_dismissed', true );
		...
		if ( $show_notice ) {
			$has_notice = true;
			?>
			<div class="challan-notice notice notice-info is-dismissible" style="line-height:1.5;" data-which="rating" data-nonce="<?php echo esc_attr( $nonce ); ?>">
				<p><?php
					printf(
					/* translators: 1: plugin name,2: Slightly Smiling Face (Emoji), 3: line break 'br' tag */
						...
						'<div class="challan-review-notice-logo"></div>',
						'<br>'
					);
					?></p>
				<p>
			...
			add_action( 'admin_print_footer_scripts', function() use ( $nonce ) {
				...
				<script>
                    (function($){
                        "use strict";
                        $(document)
                            .on('click', '.challan-notice a.button', function (e) {
                                e.preventDefault();
                                ...
                                self.closest(".challan-notice").slideUp( 200, 'linear' );
                                wp.ajax.post( 'challan_save_review_notice', { _ajax_nonce: '<?php echo esc_attr( $nonce ); ?>', notice: notice } );
                            })

                            .on('click', '.challan-notice .notice-dismiss', function (e) {
                                e.preventDefault();
                                // noinspection ES6ConvertVarToLetConst
                                var self = $(this), invoice_notice = self.closest('.challan-notice'), which = invoice_notice.attr('data-which');
                                wp.ajax.post( 'challan_hide_notice', { _wpnonce: '<?php echo esc_attr( $nonce ); ?>', which: which } );
                            });
                    })(jQuery)
				</script><?php
			}, 99 );
	 ...
	 * @return string
	 */
	public function challan_save_review_notice() {
		check_ajax_referer( 'challan_notice_nonce' );
		...
		if ( isset( $_POST['notice'] ) && ! empty( $_POST['notice'] ) && in_array( $_POST['notice'], $review_actions ) ) {
			...
			if ( 'never' === $_POST['notice'] || 'done' === $_POST['notice'] ) {
				$user_id = get_current_user_id();
				add_user_meta( $user_id, 'challan_review_notice_dismissed', true, true );
			}elseif ( 'given' !== $_POST['notice'] ) {
				update_option( 'challan_review_notice_next_show_time', time() + ( DAY_IN_SECONDS * 30 ) );
			}
			update_option( 'challan_review_notice', $value );
