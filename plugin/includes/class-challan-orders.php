 ...
 * @since 1.0.0
 *
 * @package    Challan_Orders
 * @subpackage Challan_Orders/includes
 */
...
class Challan_Orders
{
	 ...
	 *  Init Helper Class
	 *
	 * @var Challan_Helper  Helper Class.
	 */
	private $helper;
	/**
	 * Hold Order Ids
	...
	public function get_orders_info() {
		...
		foreach ( $this->order_ids as $key => $order_id ) {
			...
			if ( has_filter('challan_order_total') ) {
				$totals = apply_filters('challan_order_total', $totals, $this->template, $order);
			}

			// Packing total filter.
			if ( has_filter('challan_packing_total') ) {
				$packing_total = apply_filters('challan_packing_total', $packing_total, $this->template, $order);
			}
	...
	private function get_order_items( $order ) {
		...
		if ( has_filter('challan_product_data') ) {
			$products = apply_filters('challan_product_data', $products, $this->template, $order);
		}
	...
	private function get_order_info( $order ) {
		...
		$invoice_no = challan_get_invoice_number($order->get_id());

		$order_info['order_number']   = apply_filters('challan_order_number', $order_no, $this->template, $order);
		$order_info['order_date']     = apply_filters('challan_order_date', $order_date, $this->template, $order);
		$order_info['invoice_number'] = apply_filters('challan_invoice_number', $invoice_no, $this->template, $order);
		...
		if ( has_filter('challan_order_data') ) {
			$order_info = apply_filters('challan_order_data', $order_info, $this->template, $order);
		}
	...
	private function get_product_title( $order, $product_id, $title, $template ) {
		...
		$name .= challan_before_item_meta($product, $order, $template);
		...
		$name .= challan_after_item_meta($product, $order, $template);
...
/**
 * Initialize Challan_Orders class
 *
 * @param  array  $order_ids Order Ids.
 * @param  string $template  Template Type.
 * @return Challan_Orders
 */
function challan_orders( $order_ids, $template ) {
	return new Challan_Orders($order_ids, $template);
}
