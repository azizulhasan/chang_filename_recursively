 ...
 *
 * @package    Challan_PDF
 * @subpackage Challan_PDF/includes
 */
...
class Challan_PDF
{
    ...
    public function mpdf_config() {
        ...
        $this->config['fontdata']                 = apply_filters('challan_pdf_font_data', $font_data, $this->template);
	    ...
	    $this->config['default_font'] = apply_filters('challan_default_fonts_name', $challan_defualt_fonts);

        return apply_filters('challan_mpdf_settings', $this->config);
    }
    ...
    public function generate_pdf() {
        try {
            ...
            if ( 'save' === $this->template ) { // Save Invoice before email.
                ...
                $filename = apply_filters('challan_file_name', $filename, $template, $order_id);
                return $mpdf->Output(CHALLAN_DIR . $filename . '.pdf', 'F');
            } else {
            ...
            if ( 'save' !== $this->template ) {
                ...
                if ( strpos($pdf_error_message, 'Cannot find TTF') !== false ) {
                    $doc_link = '<a target="_blank" href="https://webappick.com/docs/challan/installation-setup/font-installation/">' .esc_html('Click Here').'</a>'; //phpcs:ignore
    ...
    private function get_watermark() {
       ...
       // Custom filter to change invoice and packing slip background.
	    $watermark_image = apply_filters('challan_background_watermark', $background, $this->template);
	    $watermark_opacity = apply_filters('challan_background_watermark_opacity', $opacity, $this->template);
    ...
    private function paper_size( $paper_size ) {
        ...
        return apply_filters('challan_paper_size', $this->paper_size, $this->template);
    }
}

/**
 * Initialize Challan_PDF class into this function.
 *
 * @param  string  $html       Document Content.
 * @param  string  $file_name  Document Name.
 * @param  integer $order_id   Order Id.
 ...
 * @return Challan_PDF
 */
function challan_pdf( $html, $file_name, $order_id, $template, $paper_size = 'A4' ) {
    return new Challan_PDF($html, $file_name, $order_id, $template, $paper_size);
}
