...
class Challan_Engine
{
    ...
    public function generate_invoice( $order_ids ) {
        ...
        $template = challan_template($ids, 'invoice');
        ...
        $pdf       = challan_pdf($html, $file_name, $order_ids, 'invoice');
        $pdf->generate_pdf();
        exit;
    }
    ...
    public function generate_packing_slip( $order_ids ) {
        $ids      = explode(',', $order_ids);
        $template = challan_template($ids, 'packing_slip');
        ...
        $pdf       = challan_pdf($slip, $file_name, $order_ids, 'packing_slip');
        $pdf->generate_pdf();
        exit;
    }
	...
	public function generate_delivery_address( $order_ids ) {
		...
		$ids      = explode(',', $order_ids);
		$template = challan_template($ids, 'label');
		...
		$pdf       = challan_pdf($label, $file_name, $order_ids, 'label');
		$pdf->generate_pdf();
		exit;
	}
    ...
    public function save_invoice( $order_id ) {
        $template = challan_template(array( $order_id ), 'invoice');
        ob_start();
        echo $template->get_invoice_template();//phpcs:ignore
        $html = ob_get_contents();
        ob_end_clean();
        ...
        $file_name = ( get_option('wpifw_INVOICE_TITLE') ) ? get_option('wpifw_INVOICE_TITLE') : 'Invoice';
        $pdf       = challan_pdf($html, $file_name, $order_id, 'save');
        $pdf->generate_pdf();
    }
}
 ...
 *
 * @return Challan_Engine
 */
function challan_engine() {
    return new Challan_Engine();
}
