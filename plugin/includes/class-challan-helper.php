...
class Challan_Helper
{
    ...
    public function get_order_number( $order ) {
        ...
        $order_no = challan_process_date_macros($order->get_id(), $order_no);
    ...
    public function get_address( $order, $type, $template, $column = null ) {
        ...
        if ( 'billing' === $type ) { // Get Billing Address.
	        ...
	        if ( empty( get_option('wpifw_display_phone' ) ) && 1 != get_option('wpifw_display_phone' ) ) {
		        echo '<p>' . challan_filter_label('Phone', $order, $template) . ' : ' . $order->get_billing_phone() . '<p>'; //phpcs:ignore
	        }
	        if ( empty( get_option('wpifw_display_email' ) ) && 1 != get_option('wpifw_display_email' ) ) {
		        echo '<p>' . challan_filter_label('Email', $order, $template) . ' : ' . $order->get_billing_email() . '<p>'; //phpcs:ignore
	        }
...
//            }
    ...
    public function get_invoice_logo() {
        ...
        $logo_url = apply_filters('challan_store_logo', $logo_url);
 ...
 *
 * @return Challan_Helper
 */
function challan_helper() {
    return new Challan_Helper();
}
